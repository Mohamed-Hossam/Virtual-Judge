<?php
	...
	class statusController extends postsoutionController
	{
		private $MODEL;
		...
		private function get_last_uva($num)
		{
			...
			while(true)
			{
				...
				for($j=1;$j<count($table);$j++)
				{
					...
					if($num==0)return array_reverse($ret);
				}
				$i+=50;
			}
		}
		...
		private function get_last_codeforces($num)
		{
			...
			if($result["status"]=="OK")
			{
				for($i=0;$i<$num;$i++)
				{
					if(!isset($result["result"][$i]["verdict"]))return array_reverse($ret);
		...
		private function get_last_spoj($num)
		{
			...
			while(true)
			{
				...
				for($j=1;$j<count($table);$j++)
				{
					...
					if(is_numeric(trim($verdict)))$verdict='accepted';
					...
					$memory=trim($memory);
					if($memory=='-')$memory=0;
					else $memory=substr($memory,0,strlen($memory)-1);
